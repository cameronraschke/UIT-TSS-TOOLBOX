      ...
      <div class='column'>
        ...
        if (strFilter($db->get()) === 0) {
          ...
          foreach ($db->get() as $key => $image) {
              ...
              if (preg_match('/^image\/.*/', $image["mime_type"]) === 1) {
                echo "<img id='" . htmlspecialchars($image["uuid"]) . "' style='max-height:100%; max-width:100%; cursor: pointer;' onclick=\"window.open('/view-images.php?view=1&tagnumber=" . $image["tagnumber"] . "&uuid=" . htmlspecialchars($image["uuid"]) . "', '_blank');\" src='data:image/jpeg;base64," . $image["thumbnail"] . "'></img>";
