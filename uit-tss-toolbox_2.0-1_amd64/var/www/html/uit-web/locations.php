...
if ($_GET["refresh"] == "1") {
  $ch = curl_init();
  curl_setopt($ch, CURLOPT_URL, "https://localhost:1411/api/refresh-client.php?password=UIT_DB_CLIENT_PASSWD&tagnumber=refresh-all");
...
if (isset($_POST["tagnumber"]) && isset($_POST['serial']) && isset($_POST["location"])) {
  ...
  if ($_POST["print"] == "1") {
    ...
    System("bash /var/www/html/uit-web/bash/uit-print-pdf" . " " . escapeshellarg("UIT_WEB_SVC_PASSWD") . " " . $tagNum . " " . $customerName . " " . $checkoutDate . " " . $returnDate);
  }
...
foreach ($tableArr as $key => $value1) {
  ...
  // Os installed
	if ($value1["os_installed"] === true && strFilter($value1["domain"]) === 0) {
		echo "<td>" . htmlspecialchars($value1['os_installed_formatted']) . "<img style='width: auto; height: 1.5em;' src='/images/intune-joined.svg'>" . "</td>" . PHP_EOL;
	} else {
