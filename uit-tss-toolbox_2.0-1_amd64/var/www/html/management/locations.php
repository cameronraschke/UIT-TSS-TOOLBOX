    ...
    <body>
        ...
        if (!empty($_POST['tagnumber'])) {
            ...
            if (!empty($arr)) {
                ...
                echo "<label for='note'>Note</label>" . PHP_EOL;
                echo "<br>" . PHP_EOL;
                dbSelectVal("SELECT note AS result FROM locations WHERE tagnumber = '" . $tagNum . "' AND note IS NOT NULL ORDER BY time DESC LIMIT 1");
                echo "<input type='text' id='note' name='note' value = '" . $result . "'>" . PHP_EOL;
                echo "<br>" . PHP_EOL;
                echo "<input type='submit' value='Update Location'>" . PHP_EOL;
                echo "<input type='hidden' name='status' value='" . $_POST['status']. "'>";
                echo "</form>" . PHP_EOL;
