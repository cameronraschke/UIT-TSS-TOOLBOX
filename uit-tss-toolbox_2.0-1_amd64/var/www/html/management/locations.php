        ...
        if (!empty($_POST['tagnumber'])) {
            ...
            if (!empty($arr)) {
                foreach ($arr as $key => $value) {
                    ...
                    $department = $result;
                    dbSelectVal("SELECT (CASE WHEN department = 'techComm' THEN 'Tech Commons (TSS)' WHEN department = 'property' THEN 'Property' WHEN deparment = 'shrl' THEN 'SHRL (Kirven)' END) AS 'result' FROM jobstats WHERE tagnumber = '" . $_POST['tagnumber'] . "' AND department IS NOT NULL ORDER BY time DESC LIMIT 1");
                    ...
                    echo "<br>" . PHP_EOL;
                    echo "<input type='text' id='location' name='location' value='" . htmlspecialchars($value['location']) . "' autofocus required>" . PHP_EOL;
                    echo "<br>" . PHP_EOL;
