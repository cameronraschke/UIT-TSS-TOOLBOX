...
<html>
    ...
    <script>
        ...
        <table width="100%">
            ...
            <tbody>
                <?php
                $db->Pselect("SELECT DATE_FORMAT(time, '%b %D %Y, %r') AS 'time_formatted', location, IF (status = 1, 'Broken', 'Working') AS 'status', IF (os_installed = 1, 'Yes', 'No') AS 'os_installed', IF (bios_updated = 1, 'Yes', 'No') AS 'bios_updated', IF (disk_removed = 1, 'Yes', 'No') AS 'disk_removed', note FROM locations WHERE (time <= NOW() - INTERVAL 12 MONTH) AND time IN (SELECT MAX(time) FROM locations WHERE tagnumber = :tagnumber GROUP BY tagnumber) ORDER BY time DESC", array(':tagnumber' => htmlspecialchars_decode($_GET["tagnumber"])));
                if (arrFilter($db->get()) === 0) {
                    foreach ($db->get() as $key=>$value) {
                        echo "<tr>" . PHP_EOL;
                        //Time formatted
